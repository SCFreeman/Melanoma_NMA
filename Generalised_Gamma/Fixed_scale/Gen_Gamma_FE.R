...
BUGS_data<-list(LnObs=dim(study_GG)[1], nTx=nTreat, nStudies=nStudies, Lstudy=as.numeric(study_GG$studyCode),
                ...
                Lmean2=study_GG[,"sigma"], Lse2=study_GG[,"sigma.se"], mean=mean, prec=prec)

# Set up initial values for FE model
initsFE1<-list(alpha = rep(-0.5,nStudies), beta = array(c(NA, rep(-0.5,nTreat-1), NA, rep(-0.5,nTreat-1)), dim=c(nTreat,2)))
initsFE2<-list(alpha = rep(0.5,nStudies), beta = array(c(NA, rep(0.5,nTreat-1), NA, rep(0.5,nTreat-1)), dim=c(nTreat,2)))
initsFE3<-list(alpha = rep(0.1,nStudies), beta = array(c(NA, rep(0.1,nTreat-1), NA, rep(0.1,nTreat-1)), dim=c(nTreat,2)))
...
nmaAggrFE<-bugs(data=BUGS_data, inits=initsFE, parameters.to.save=c("beta", "aft", "rk.beta1", "rk.beta2",
                                                                    "scale.aft"),
...
# Store trt effects (beta[,1]) for each treatment
trt2 <- nmaAggrFE$summary[1,1]
trt3 <- nmaAggrFE$summary[3,1]
trt4 <- nmaAggrFE$summary[5,1]
trt5 <- nmaAggrFE$summary[7,1]
trt6 <- nmaAggrFE$summary[9,1]
trt7 <- nmaAggrFE$summary[11,1]
trt8 <- nmaAggrFE$summary[13,1]
trt9 <- nmaAggrFE$summary[15,1]
trt10 <- nmaAggrFE$summary[17,1]
trt11 <- nmaAggrFE$summary[19,1]
trt12 <- nmaAggrFE$summary[21,1]
trt13 <- nmaAggrFE$summary[23,1]

# Store sigma effects (beta[,2]) for each treatment
sigma.trt2 <- nmaAggrFE$summary[2,1]
sigma.trt3 <- nmaAggrFE$summary[4,1]
sigma.trt4 <- nmaAggrFE$summary[6,1]
sigma.trt5 <- nmaAggrFE$summary[8,1]
sigma.trt6 <- nmaAggrFE$summary[10,1]
sigma.trt7 <- nmaAggrFE$summary[12,1]
sigma.trt8 <- nmaAggrFE$summary[14,1]
sigma.trt9 <- nmaAggrFE$summary[16,1]
sigma.trt10 <- nmaAggrFE$summary[18,1]
sigma.trt11 <- nmaAggrFE$summary[20,1]
sigma.trt12 <- nmaAggrFE$summary[22,1]
sigma.trt13 <- nmaAggrFE$summary[24,1]
